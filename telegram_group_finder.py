...
# По умолчанию список ключевых слов (используем для примера, но в боте ищем по переданному слову)
DEFAULT_KEYWORDS = [
...
async def search_groups(client: TelegramClient, keyword: str) -> List[dict]:
    """Ищем группы по ключевому слову."""
    found = {}
...
async def check_group_for_bots(client: TelegramClient, chat_info: dict) -> dict:
    """Проверяем первых участников группы на наличие ботов."""
...
async def main_bot():
    ...
    # Хранение результатов поиска (последний запрос)
    last_results = []
